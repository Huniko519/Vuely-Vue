<template>
    <v-menu
            ...
            offset-y
            left
            origin="right top" z-index="99" content-class="cart-dropdown" transition="slide-y-transition"
            nudge-top="-20"
    >
        ...
        <v-card>
            ...
            <v-card-actions v-if="cart != ''">
                <v-btn color="primary" v-if="horizontal" small
                       :to="`/${getCurrentAppLayoutHandler() + '/ecommerce/cart'}`">
                    {{$t('message.viewCart')}}
                </v-btn>
                <v-btn v-else small color="primary" :to="`/${getCurrentAppLayoutHandler() + '/ecommerce/cart'}`">
                    {{$t('message.viewCart')}}
                </v-btn>
                <v-spacer></v-spacer>
