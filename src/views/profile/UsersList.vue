            ...
            <v-layout row wrap v-if="usersList !== null">
                ...
                >
                    ...
                    <div class="user-list-content">
                        <div class="text-xs-center">
                            ...
                            <div class="social-list clearfix mb-4">
                                <ul class="list-inline d-inline-block">
                                    <li v-for="(links, index) in users.socialLinks" :key="index">
                                        <a :href="links.url" class="text-pink">
                        ...
                        <div class="layout justify-space-between border-tb-1 pa-2 mb-3">
                            ...
                            <div class="align-item-end">
                                <ul class="list-inline">
                                    <li
                                        class="pa-0"
                                        v-for="(user, index) in users.connections"
                                        :key="index"
                                    >
                                        <img
                                            :src="getImgSrc(user)"
                        ...
                        <a
                            class="v-btn btn-success white--text ma-0"
                            href="javascript:void(0);"
                            @click="users.status = !users.status"
                            v-if="users.status"
                        >{{$t("message.follow")}}</a>
                        <a
                            class="v-btn btn-danger white--text ma-0"
                            href="javascript:void(0);"
                            @click="users.status = !users.status"
                            v-else
                        >{{$t("message.unfollow")}}</a>
                    </div>
                </app-card>
            </v-layout>
        </v-container>
...
export default {
    ...
    methods: {
        ...
        getUsers() {
            ApiService.get("vuely/users.js").then(response => {
                this.usersList = response;
            });
        },
        getConnections() {
            ApiService.get("vuely/connections.js")
                .then(response => {
                    this.connectUsersList = response;
                })
                .catch(error => {
                    console.log(error);
                });
